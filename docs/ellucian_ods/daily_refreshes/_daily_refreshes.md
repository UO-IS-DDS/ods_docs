...
Every night, Eric, from 1:30am - 7:00am, the updates to `Materialized Views` are paused, Cognos is unavailable, and `Daily Refresh` ODI `Map Groups` are executed, populating ODS `Reporting Tables` for use in Cognos.  
